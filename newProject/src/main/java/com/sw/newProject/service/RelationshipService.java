...
public class RelationshipService {
    ...
    @Transactional
    public void doFollow(MemberDto reqMember, Integer accMember) {
        log.info("reqMember: {}", reqMember.getMemNo());
        log.info("accMember: {}", accMember);

        List<String> reqMemberFollowData = getJsonKeysList(reqMember.getMemNo()); // 요청자의 현재 팔로우 데이터를 가져오기
        List<String> accMemberFollowingData = getJsonKeysList(accMember); // 수락자의 현재 팔로잉 데이터를 가져오기

        if (reqMemberFollowData.contains(accMember.toString())) { // 요청자의 팔로우 데이터 중 수락자가 존재하는 지 확인
            ...
            appendTargetToJsonDto.setTargetMemNo(accMember);
            appendTargetToJsonDto.setColumnName("follow");
            appendMemNoToJson(appendTargetToJsonDto);
        }

        if (accMemberFollowingData.contains(reqMember.toString())) { // 수락자의 팔로잉 데이터 중 요청자가 존재하는 지 확인
            ResponseEntity.badRequest();
        } else { // 수락자 팔로잉 데이터에 요청자를 추가
            AppendTargetToJsonDto appendTargetToJsonDto = new AppendTargetToJsonDto();
            appendTargetToJsonDto.setOwnerMemNo(accMember);
